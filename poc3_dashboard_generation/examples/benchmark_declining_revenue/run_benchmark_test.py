...
synthetic declining-revenue benchmark dataset (see generate_data.py's module docstring for the
dataset's deliberate shape: revenue/order/customer growth all clearly negative, repeat_purchase_rate
kept unremarkable). Same wiring pattern as examples/real_olist_integration/run_real_test.py, minus
the real-data judgment calls that script needs (this dataset's registry is just
`bootstrap_registry()`, unchanged, since it uses the same entity shape as this POC's own `data/`).
...
def main():
    ...
    print("expected shape: revenue_growth={:.2f}, order_growth={:.2f}, customer_growth={:.2f}, repeat_purchase_rate={:.2f} (a clear, consistent decline)\n".format(
