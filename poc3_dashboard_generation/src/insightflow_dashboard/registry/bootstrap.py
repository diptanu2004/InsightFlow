# Vendored, unmodified, from poc2_analytics_engine/src/insightflow/registry/bootstrap.py.
# Same vendoring rationale as pipeline.py's header comment -- POC 3's SignalGatherer and
# DashboardPlanner both need the real, already-tested ten-metric registry, not a reimplementation.
"""Registers the ten target metrics from docs/supported_metrics.md into a fresh MetricRegistry.

This is POC2's own configuration, not derived from the Semantic Model — it encodes which raw
fields (`Measure.source_field`) map to which Tier 1 measures and Tier 2 metrics for the sample
dataset in `data/`. A real deployment would generate/adjust this per business, but for POC2 the
target metric list is fixed (hld.md's `docs/supported_metrics.md`).

Assumes an `orders` entity with fields `order_id`, `customer_id`, `revenue`, `transaction_date`
(POC 1's real canonical vocabulary name for dates — see `data/semantic_model.json` and
`docs/supported_metrics.md`'s build order: three base measures + dimension grouping first, then
growth, then the HAVING/threshold operator).
"""
